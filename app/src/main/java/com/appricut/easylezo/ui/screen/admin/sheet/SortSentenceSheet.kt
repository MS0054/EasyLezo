...
fun SortSentenceSheet(
    ...
    onDismiss: () -> Unit,
    onSave: (List<Sentence>) -> Unit
) {
    val sheetState = rememberModalBottomSheetState(skipPartiallyExpanded = true)

    // کپی مستقل برای reorder
    val list = remember {
        mutableStateListOf<Sentence>().apply {
            addAll(sentences.sortedBy { it.order })
        }
    }
    ...
    ModalBottomSheet(
        ...
        modifier = Modifier.fillMaxSize() // فول‌اسکرین
    ) {
        Column(Modifier.fillMaxSize()) {
            ...
            DraggableLazyColumn(
                ...
                SentenceDragItem(sen, isDragging)
            }
            ...
            Row(
                ...
                Button(onClick = {
                    val reordered = list.mapIndexed { index, sen ->
                        sen.copy(order = index)
                    }
                    onSave(reordered)
                }) {
                    Text("Save")
...
// ===== آیتم Sentence با Shadow + Scale وقتی در حال Drag =====
@Composable
