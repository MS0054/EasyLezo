...
fun EditErrorSheet(
    ...
    val translationMap = remember { mutableStateMapOf<String, String>().apply {
        error.translations.forEach { put(it.language, it.text) }
    } }
    var code by remember { mutableStateOf(error.code) }


    Column(Modifier.fillMaxWidth().padding(20.dp)) {
        Row (Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            Text("Edit Error", style = MaterialTheme.typography.headlineSmall)
            Row {
                IconButton(onClick = { onDelete(error) }) { Icon(Icons.Default.Delete, tint = Color.Red, contentDescription = null) }
                Button(onClick = {
