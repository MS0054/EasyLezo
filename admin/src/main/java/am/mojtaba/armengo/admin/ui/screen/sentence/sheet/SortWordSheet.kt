...
fun SortWordSheet(
    words: List<Word>,
    onDismiss: () -> Unit,
    onSubmit: (List<Word>) -> Unit
) {
    val list =
        remember { mutableStateListOf<Word>().apply { addAll(words.sortedBy { it.order }) } }
    ...
    Column(Modifier.fillMaxWidth().padding(20.dp)) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            ...
            Button(onClick = {
                val reordered = list.mapIndexed { index, cat -> cat.copy(order = index) }; onSubmit(
                reordered
            )
            }) { Text("Save") }
        }
...
@Composable
fun WordDragItem(word: Word, isDragging: Boolean) {
...
fun DraggableLazyColumn(
    items: SnapshotStateList<Word>,
    onMove: (fromIndex: Int, toIndex: Int) -> Unit,
    itemContent: @Composable (Word, Boolean) -> Unit
) {
