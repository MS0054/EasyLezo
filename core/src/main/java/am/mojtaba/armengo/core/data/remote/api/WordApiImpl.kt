...
class WordApiImpl @Inject constructor(
    ...
    private val wordCol = db.collection(COLLECTION)

    override suspend fun getWords(): List<WordDto> {
        val snap = wordCol.orderBy(ORDER_FIELD).get().await()
        return snap.documents.mapNotNull { doc ->
            doc.toObject(WordDto::class.java)?.copy(id = doc.id)
        }
    }

    override suspend fun syncWords(words: List<WordDto>) {
        if (words.isEmpty()) return

        val batch = db.batch()
        words.forEach { dto ->
            val docRef = wordCol.document(dto.id)
            if (dto.isDeleted) {
                batch.delete(docRef)
            } else {
    ...
    override suspend fun downloadVoices(words: List<WordDto>) {
